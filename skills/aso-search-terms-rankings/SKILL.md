...
- Prefer available read-only tools that can inspect App Store Connect or public App Store listing data. For example, use the [Helm CLI](https://nielsmouthaan.dev/helm) (`helm-asc`), `asc`, or another App Store Connect-capable tool to obtain the latest live App Store version when available.
