...
- Future specialist skills should check `.agents/aso-app-context.md` before starting. If it exists, use it as canonical app context. If it is missing or incomplete, invoke or recommend `aso-app-context` first.
...
- Treat root-level shared support files as canonical.
- Copy shared files into skill-local folders only when standalone installation requires those files.
