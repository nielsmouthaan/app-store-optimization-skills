...
- Never allow an agent to write to App Store Connect through the [Helm CLI](https://nielsmouthaan.nl/helm) (`helm-asc`), `asc`, the App Store Connect API, or another tool unless the user has explicitly reviewed and approved that write.
- Keep skill-development guidance in `AGENTS.md` and skill-execution behavior in `SKILL.md`. For example, do not hardcode exact App Store Connect tool command parameters in ASO skills; instruct agents to use the current tool-specific skill, CLI help, or command discovery instead. Useful CLI examples include the [Helm CLI](https://nielsmouthaan.nl/helm) (`helm-asc`) and `asc`, but skills should remain open to user-provided or future App Store Connect-capable tools.
